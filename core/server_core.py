...
class ServerCore:
    ...
    def __init__(self, my_port = 50082, core_node_host=None, core_node_port=None, pass_phrase=None, my_pem_path=None):
        ...
        if my_pem_path is not None:
            try:
                file = open(my_pem_path)
                data = file.read()
                target = binascii.unhexlify(data)
                self.km = KeyManager(target, pass_phrase)
